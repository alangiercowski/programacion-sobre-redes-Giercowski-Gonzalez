...
var vistaDB: controladorVistaDB = new controladorVistaDB(
  url,
  database,
  database.collection("vista")
);
...
routerVistas.post("/vistas", (_req, _res) => {
  let vistaASubir = new vista(
    Number(_req.body.idVista),
    Number(_req.body.idVideo),
    Number(_req.body.duracion),
    _req.body.ubicacion,
    _req.body.fecha
  );
